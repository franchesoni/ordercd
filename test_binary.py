...
def main(n_epochs=1, seed=0, device="cuda:1"):
    ds = Sentinel2Dataset()
    ...
    transformer_layer_state_dict = torch.load(
        "runs/Feb11_11-38-43_damanbinary/transformer_layer.pth"
    )
    ...
    linear_layer_state_dict = torch.load("runs/Feb11_11-38-43_damanbinary/linear_layer.pth")
    linear_layer.load_state_dict(linear_layer_state_dict)
    linear_layer.to(device)

    times = {}
    max_change_so_far = 0
    for batch_idx, batch in enumerate(tqdm.tqdm(dl, total=len(dl), disable=True)):
